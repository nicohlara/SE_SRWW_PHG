...
dir="/90daydata/guedira_seq_map/nico/phg_LDblock"
...
for (sample in founders) {
  ...
  for (shortread in c(exome, gbs)) {
    print(shortread)
    comp_vec_genic <- c(comp_vec_genic, compare_overlap(phgDs, sample, shortread))
    print(tail(comp_vec_genic, 1))
  }
  identity_table[[glue("{sample}_genic")]] = round(as.numeric(comp_vec_genic), 2)
}
print("Haplotype identity table calculated")
